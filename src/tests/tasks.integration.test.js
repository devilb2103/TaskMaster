...
describe('Task API Integration Tests (/api/tasks)', () => {
  ...
  beforeEach(async () => {
    ...
    // Create User One
    const userOneData = { name: 'User One', email: 'userone@test.com', password: 'password1' };
    await request(app).post('/api/users/register').send(userOneData);
    const loginResOne = await request(app).post('/api/users/login').send({ email: userOneData.email, password: userOneData.password });
    userOneToken = loginResOne.body.token;
    const userOne = await User.findOne({ email: userOneData.email });
    userOneId = userOne._id;

    // Create User Two
    const userTwoData = { name: 'User Two', email: 'usertwo@test.com', password: 'password2' };
    await request(app).post('/api/users/register').send(userTwoData);
    const loginResTwo = await request(app).post('/api/users/login').send({ email: userTwoData.email, password: userTwoData.password });
  ...
  describe('POST /api/tasks', () => {
    ...
    it('should return 400 if description is missing', async () => {
       const res = await request(app).post('/api/tasks').set('Authorization', `Bearer ${userOneToken}`).send({});
       expect(res.statusCode).toEqual(400);
       expect(res.body.errors[0].msg).toContain('Description is required');
    });
  });
  ...
  describe('GET /api/tasks', () => {
     it('should get tasks only for the authenticated user (default pagination)', async () => {
        ...
        await Task.create({ description: 'User One - Second Task', owner: userOneId });
        // Add a task for user two (should not be returned)
        await Task.create({ description: 'User Two - Task X', owner: userTwoId });
        ...
        expect(res.body.tasks.every(t => t.owner === userOneId.toString())).toBe(true);
     });
     ...
     it('should handle limit and page query parameters for pagination', async () => {
        ...
        for (let i = 1; i <= 15; i++) {
            ...
            tasksToCreate.push({ description: `User One - Pagination Task ${i}`, owner: userOneId });
        }
        await Task.insertMany(tasksToCreate);
        // Add a task for user two (should not affect counts/results for user one)
        await Task.create({ description: 'User Two - Task Y', owner: userTwoId });
     ...
     it('should handle filtering with pagination', async () => {
        ...
        await Task.create({ description: 'User One - Pending 1', owner: userOneId, status: 'pending' });
        await Task.create({ description: 'User One - Pending 2', owner: userOneId, status: 'pending' });
        await Task.create({ description: 'User One - Completed 1', owner: userOneId, status: 'completed' });
        ...
        expect(res.body.tasks.every(t => t.status === 'pending')).toBe(true); // Verify status
    ...
    it('should handle sorting with pagination', async () => {
        ...
        await Task.create({ description: 'User One - Task C', owner: userOneId }); // Created after initial
        await Task.create({ description: 'User One - Task A', owner: userOneId }); // Created last
        ...
        expect(res.body.tasks[0].description).toBe('User One - Initial Task'); // I comes before P, T
  ...
  describe('GET /api/tasks/:id', () => {
    it('should get a specific task by ID if owned by the user', async () => {
        const res = await request(app).get(`/api/tasks/${taskOneId}`).set('Authorization', `Bearer ${userOneToken}`);
        expect(res.statusCode).toEqual(200);
    ...
    it('should return 401 if user tries to get a task they do not own', async () => {
        const res = await request(app).get(`/api/tasks/${taskOneId}`).set('Authorization', `Bearer ${userTwoToken}`);
        expect(res.statusCode).toEqual(401);
    });
    it('should return 404 if task ID is invalid', async () => {
        const res = await request(app).get(`/api/tasks/invalidObjectIdFormat`).set('Authorization', `Bearer ${userOneToken}`);
        expect(res.statusCode).toEqual(404);
    });
    it('should return 404 if task ID does not exist', async () => {
        const nonExistentId = new mongoose.Types.ObjectId();
        const res = await request(app).get(`/api/tasks/${nonExistentId}`).set('Authorization', `Bearer ${userOneToken}`);
        expect(res.statusCode).toEqual(404);
    });
    it('should return 401 if user is not authenticated', async () => {
  ...
  describe('PUT /api/tasks/:id', () => {
    it('should update a task if owned by the user', async () => {
        const updates = { description: 'Updated Task Desc', status: 'completed' };
        const res = await request(app).put(`/api/tasks/${taskOneId}`).set('Authorization', `Bearer ${userOneToken}`).send(updates);
        expect(res.statusCode).toEqual(200);
    ...
    it('should return 401 if user tries to update a task they do not own', async () => {
        const res = await request(app).put(`/api/tasks/${taskOneId}`).set('Authorization', `Bearer ${userTwoToken}`).send({ description: 'Attempted Update' });
        expect(res.statusCode).toEqual(401);
    });
    it('should return 401 if user is not authenticated', async () => {
        const res = await request(app).put(`/api/tasks/${taskOneId}`).send({ description: 'Unauthorized Update' });
        expect(res.statusCode).toEqual(401);
    });
    it('should return 404 if task ID does not exist', async () => {
        const nonExistentId = new mongoose.Types.ObjectId();
        const res = await request(app).put(`/api/tasks/${nonExistentId}`).set('Authorization', `Bearer ${userOneToken}`).send({ description: 'Update Non-Existent' });
        expect(res.statusCode).toEqual(404);
    });
  });
  ...
  describe('DELETE /api/tasks/:id', () => {
    it('should delete a task if owned by the user', async () => {
        const res = await request(app).delete(`/api/tasks/${taskOneId}`).set('Authorization', `Bearer ${userOneToken}`);
        expect(res.statusCode).toEqual(200);
    ...
    it('should return 401 if user tries to delete a task they do not own', async () => {
        const res = await request(app).delete(`/api/tasks/${taskOneId}`).set('Authorization', `Bearer ${userTwoToken}`);
        expect(res.statusCode).toEqual(401);
    ...
    it('should return 404 if task ID does not exist', async () => {
        const nonExistentId = new mongoose.Types.ObjectId();
        const res = await request(app).delete(`/api/tasks/${nonExistentId}`).set('Authorization', `Bearer ${userOneToken}`);
        expect(res.statusCode).toEqual(404);
    });
  });
